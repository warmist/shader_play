...
void Camera::setViewport(unsigned offsetx, unsigned offsety, unsigned width, unsigned height)
{
    mVpX = offsetx;
    mVpY = offsety;
    mVpWidth = width;
...
void Camera::rotateAroundTarget(const Quaternionf& q)
{
    ...
    mViewMatrix_inv = mViewMatrix.inverse();
...
void Camera::updateProjectionMatrix(void) const
{
  if(!mProjIsUptodate)
  {
    ...
    mProjectionMatrix_inv = mProjectionMatrix.inverse();
...
void Camera::activateGL(int projection, int modelview,int p_inv,int mv_inv)
{
  //glViewport(vpX(), vpY(), vpWidth(), vpHeight());
    glUniformMatrix4fv(projection, 1, projectionMatrix().IsRowMajor == 1, projectionMatrix().data());
    glUniformMatrix4fv(modelview, 1, viewMatrix().matrix().IsRowMajor == 1, viewMatrix().matrix().data());
    glUniformMatrix4fv(p_inv, 1, mProjectionMatrix_inv.IsRowMajor == 1, mProjectionMatrix_inv.data());
    glUniformMatrix4fv(mv_inv, 1, mViewMatrix_inv.matrix().IsRowMajor == 1, mViewMatrix_inv.matrix().data());
}
