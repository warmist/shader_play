...
class Camera
{
  public:
    ...
    void setViewport(unsigned offsetx, unsigned offsety, unsigned width, unsigned height);
    void setViewport(unsigned width, unsigned height);
    
    inline unsigned vpX(void) const { return mVpX; }
    inline unsigned vpY(void) const { return mVpY; }
    inline unsigned vpWidth(void) const { return mVpWidth; }
    inline unsigned vpHeight(void) const { return mVpHeight; }
    ...
    void activateGL(int projection, int modelview, int p_inv, int mv_inv);
  ...
  protected:
    ...
    unsigned mVpX, mVpY;
    unsigned mVpWidth, mVpHeight;

    Frame mFrame;
    
    mutable Eigen::Affine3f mViewMatrix;
    mutable Eigen::Matrix4f mProjectionMatrix;
    mutable Eigen::Affine3f mViewMatrix_inv;
    mutable Eigen::Matrix4f mProjectionMatrix_inv;
